...
| Operation | Path |
|-----------|------|
| Read | `students/{student_id}/profile.json` (primary) |
| Read | `students/{student_id}/history.json` (primary) |
| Read | `memory/students/{student_id}/profile.json` (fallback) |
| Read | `memory/students/{student_id}/history.json` (fallback) |

**Note**: Check `students/` first, fall back to `memory/students/` for compatibility.
